...
unsafe impl<'a> SingularGeneric for &'a str {
    type Repr = &'a str;
}

unsafe impl<T: SingularGeneric> SingularGeneric for Box<[T]> {
    type Repr = Box<[T::Repr]>;
}

unsafe impl<T: SingularGeneric> SingularGeneric for Vec<T> {
...
impl<T: SingularGeneric<Repr = JsValue> + WasmDescribe> VectorFromWasmAbi for T {
    type Abi = WasmSlice;
...
impl<T: SingularGeneric<Repr = JsValue> + WasmDescribe> VectorIntoWasmAbi for T {
    type Abi = WasmSlice;
...
impl<T: SingularGeneric<Repr = JsValue> + WasmDescribe> IntoWasmAbi for &[T] {
    type Abi = WasmSlice;
...
impl<T: SingularGeneric<Repr = JsValue> + WasmDescribe> OptionIntoWasmAbi for &[T] {
    #[inline]
    fn none() -> WasmSlice {
        null_slice()
    }
