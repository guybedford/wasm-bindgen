...
// Blanket implementation of JsUpcastRef for &T
// impl<T> JsUpcastRef for &T
// where
//     T: JsUpcastRef + ?Sized,
// {
//     const USES_VALUE_SLOTS: bool = T::USES_VALUE_SLOTS;
...
impl JsUpcastRef for i64 {
    #[inline]
    fn upcast_ref(&self) -> &JsValue {
        panic!("upcast_ref_mut not supported for bigint types - use upcast_into instead")
...
impl JsUpcastRef for char {
    #[inline]
    fn upcast_ref(&self) -> &JsValue {
        panic!("upcast_ref not supported for string types - use upcast_into instead")
        // let s = self.to_string();
        // let slot_idx = unsafe { crate::__wbindgen_string_new_into_slot(s.as_ptr(), s.len()) };
        // __rt::get_value_slot(slot_idx)
    }
}
