...
macro_rules! type_wasm_native_f64_option {
    ($($t:tt as $c:tt)*) => ($(
        ...
        unsafe impl SingularGeneric for $t {
            type Repr = $t;
        }
...
unsafe impl SingularGeneric for bool {
    type Repr = bool;
}
...
unsafe impl SingularGeneric for char {
    type Repr = char;
}
...
unsafe impl<T: SingularGeneric> SingularGeneric for *const T {
...
unsafe impl SingularGeneric for () {
    type Repr = ();
}
...
unsafe impl<T: SingularGeneric, E: SingularGeneric> SingularGeneric for Result<T, E> {
    type Repr = Result<<T as SingularGeneric>::Repr, <E as SingularGeneric>::Repr>;
}
