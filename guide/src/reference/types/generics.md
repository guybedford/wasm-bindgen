...
* `ArrayTuple<T1, ..., T8>`: JS Array builtin variant with typing per item up to 8 items.
* `Function<Ret, A1, ..., A8>`: JS function external type, with a typed return and up to 8 arguments. See [Function Type Aliases](#function-type-aliases) for more details.
...
// Convert to Option for Rust-style handling
match value.into_option() {
    Some(num) => println!("Got number: {}", num.value_of()),
...
let value = maybe_get_value();
let num = value.unwrap();                           // panic if null
let num = value.expect("value should exist");       // panic with message
let num = value.unwrap_or_default();                // use T::default() if null
let num = value.unwrap_or_else(|| Number::from(0)); // compute default if null
...
The `Function` type has several aliases:

* **`TypedFunction`**: `Function` with strict arity checking—arguments default to `None`, so `call4()` won't compile unless the function has at least 4 declared arguments. This will become the default `Function` behavior in the next major release.

* **`AnyFunction`**: Currently just an alias for `Function`. Use this to preserve lenient behavior (all params default to `JsValue`) after the next major release.

* **`VoidFunction`**: `Function<Undefined, ...>` with strict arity checking, for functions with no return value.

### Calling and Binding Functions with Tuples

The `calln` and `bindn` methods allow calling or binding functions with arguments provided as a tuple of references:

```rust
use js_sys::{Function, Number, JsString, Boolean};

let f: Function<Number, JsString, Boolean> = get_function();

// Call with no args
let result = f.calln(&context, ())?;

// Call with one arg (use 1-tuple)
let result = f.calln(&context, (&my_string,))?;

// Call with two args
let result = f.calln(&context, (&my_string, &my_bool))?;

// Subtypes work - pass &Number where &JsValue is expected
let f2: Function<Number, JsValue> = get_function();
let result = f2.calln(&context, (&my_number,))?;
```

The `bindn` method works similarly but returns a new function with the arguments pre-bound:

```rust
// Bind one argument
let bound: Function<Number, Boolean> = f.bindn(&context, (&my_string,));

// Bind two arguments
let bound: Function<Number> = f.bindn(&context, (&my_string, &my_bool));
```
...
The `JsGeneric` trait is a shorthand for `ErasableGeneric<Repr = JsValue>`. This is the primary generic repr used for JavaScript interop, and generic type parameters must be congruent—they must share the same `Repr` to be used together.
