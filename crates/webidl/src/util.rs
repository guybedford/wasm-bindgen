...
/// From `base_path` and `T1, T2` create `base_path<T1, T2>`, unless both are JsValue
/// For example: `js_sys::ArrayTuple` + `i32, String` → `js_sys::ArrayTuple<i32, String>`
/// But: `js_sys::ArrayTuple` + `JsValue, JsValue` → `js_sys::ArrayTuple`
pub(crate) fn generic_ty2(
    base: syn::Type,
    type_arg1: syn::Type,
    type_arg2: syn::Type,
) -> syn::Type {
    // If both inner types are JsValue, omit the generics
    if is_js_value(&type_arg1) && is_js_value(&type_arg2) {
        return base;
    }
