...
impl ast::ImportFunction {
    ...
    fn try_to_tokens_generic(&self, tokens: &mut TokenStream) -> Result<(), Diagnostic> {
        ...
        // POC: class-level generics (`this: &Foo<T>`) are supported by hoisting
        // the class-typed parameters onto a parameterised `impl` block, reusing
        // the same `get_fn_generics` partition the erased path uses. The
        // imported type's handle impls (`IntoWasmAbi for Foo<T>` etc.) carry no
        // erasure bound, so any `T` marshals as the class handle; only the
        // per-mono `t: T` arguments constrain `T` (via the synthesized
        ...
        // collected here.
        // POC: with a parameterised class impl, the hoisted params are declared
        // on the `impl` header as bare idents, so every user bound (inline and
        ...
        for (i, arg) in self.function.arguments.iter().enumerate() {
            ...
            if generics::uses_generic_params(ty, &type_params) {
                ...
                // POC: a closure argument (`&dyn Fn(..)` / `&mut dyn FnMut(..)`)
        ...
        // POC: params hoisted to a parameterised class impl are excluded from
        // the method's own generics (redeclaring them would be E0403); the rest
        ...
        if let Some(class) = class {
            ...
            if class_is_generic {
                // POC: parameterised impl mirroring the erased path's
