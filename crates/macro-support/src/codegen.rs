    ...
    fn try_to_tokens(&self, tokens: &mut TokenStream) -> Result<(), Diagnostic> {
            ...
            if generics::uses_generic_params(ty, &fn_generic_param_names) {
                ...
                if i > 0 || !is_method {
                    ...
                    fn_class_generics.add_fn_bound(if let Some(mut_) = ref_mut {
                        if mut_.is_some() {
                            parse_quote! { #inner_ty: #wasm_bindgen::__rt::marker::GenericBorrowMut<#concrete_ty> }
                        } else {
                            parse_quote! { #inner_ty: #wasm_bindgen::__rt::marker::GenericBorrow<#concrete_ty> }
                        }
                    } else {
                        parse_quote! { #inner_ty: #wasm_bindgen::__rt::marker::GenericOwn<#concrete_ty> }
                    });
                }
        ...
        if self.function.r#async {
            ...
            match &self.js_ret {
                ...
                Some(ref ty) => {
                    fn_class_generics
                        .add_fn_bound(parse_quote! { #ty: #wasm_bindgen::convert::FromWasmAbi });
                ...
                Some(ty) => {
                    if generics::uses_generic_params(ty, &fn_generic_param_names) {
                        ...
                        fn_class_generics.add_fn_bound(
                            parse_quote! { #ty: #wasm_bindgen::__rt::marker::GenericOwn<#concrete_ty> },
                        );
