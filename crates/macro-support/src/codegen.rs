    ...
    fn get_fn_generics<'a>(&'a self) -> Result<FnClassGenerics<'a>, Diagnostic> {
        ...
        if let Some(cls_path) = class {
            ...
            }) = cls_path.segments.last()
            {
                ...
                // Transitively hoist generic params that are used in bounds OF already-hoisted params.
                // For example, if F is hoisted and has bound `F: JsFunction<Ret = Ret>`, then Ret
                // must also be hoisted since it appears in a bound on F.
                            ...
                            if let syn::Type::Path(type_path) = &pred_type.bounded_ty {
                                if type_path.qself.is_none() && type_path.path.segments.len() == 1 {
                                    ...
                                    if class_generic_params.contains(bounded_ident) {
                                        // This bound is ON a class param, check for fn params used in the bounds
                    ...
                    if let Some(inner_ty) = is_as_upcast_impl(&arg.pat_type.ty) {
                        ...
                        if generics::uses_generic_params(&inner_ty, &class_generic_params_refs) {
                                ...
                                generics::is_as_upcast_impl_raw(&arg.pat_type.ty)
                            {
                                ...
                                // Only add to bound-only set if not already in the type lifetime set
