...
// Remaining generics features still to implement:
...
struct GenericRenameVisitor<'a> {
    renames: &'a HashMap<&'a Ident, Cow<'a, syn::Type>>,
}
    ...
    fn visit_type_mut(&mut self, ty: &mut Type) {
        if let Type::Path(type_path) = ty {
            ...
            if let Some(qself) = &mut type_path.qself {
                if let Type::Path(qself_path) = &mut *qself.ty {
                    if qself_path.qself.is_none() && qself_path.path.segments.len() == 1 {
                        ...
                        if let Some(concrete) = self.renames.get(ident) {
                            *qself.ty = concrete.clone().into_owned();
                            return;
                        }
                    }
                }
            ...
            if type_path.qself.is_none() && !type_path.path.segments.is_empty() {
                ...
                if let Some(concrete) = self.renames.get(&first_seg.ident) {
...
impl<'a, 'b> GenericNameVisitor<'a, 'b> {
    ...
    pub fn new(generic_params: &'a Vec<&'a Ident>, found_set: &'b mut HashSet<Ident>) -> Self {
        Self {
            generic_params,
            found_set,
        }
...
pub(crate) fn generic_where_rename<'a>(
    mut predicate: syn::WherePredicate,
    renames: &HashMap<&'a Ident, Cow<'a, Type>>,
) -> syn::WherePredicate {
    if renames.is_empty() {
        return predicate;
    }
    let mut visitor = GenericRenameVisitor { renames };
...
/// Renaming visitor for arbitrary generic type
pub(crate) fn generics_rename<'a>(
    mut ty: syn::Type,
    renames: &HashMap<&'a Ident, Cow<'a, Type>>,
) -> syn::Type {
    let mut visitor = GenericRenameVisitor { renames };
...
pub(crate) fn generic_to_concrete<'a>(
    mut ty: syn::Type,
    generic_names: &HashMap<&'a Ident, Option<Cow<'a, syn::Type>>>,
) -> syn::Type {
    if generic_names.is_empty() {
        return ty;
    }
    let js_value: syn::Type = syn::parse_quote!(JsValue);
    let generics: HashMap<&Ident, Cow<'_, syn::Type>> = generic_names
        .iter()
        .map(|(&ident, opt_ty)| {
            (
                ident,
                opt_ty
                    .as_deref()
                    .map(Cow::Borrowed)
                    .unwrap_or(Cow::Borrowed(&js_value)),
            )
        })
        .collect();
    let mut visitor = GenericRenameVisitor { renames: &generics };
    visitor.visit_type_mut(&mut ty);
    ty
}
...
mod tests {
    ...
    fn test_generic_args_to_concrete() {
        ...
        let generic_names: HashMap<&syn::Ident, Option<Cow<syn::Type>>> = {
